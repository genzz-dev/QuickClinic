...
const PatientAppointments = () => {
  ...
  const categorizedAppointments = useMemo(() => {
    ...
    return {
      upcoming: filteredAppointments.filter(apt => {
        ...
        return isFuture(aptDate) || (isToday(aptDate) && differenceInHours(new Date(`${apt.date}T${apt.startTime}`), today) > 0);
  ...
  const AppointmentCard = ({ appointment }) => {
    const { date: formattedDate, time: formattedTime } = formatDateTime(appointment.date, appointment.startTime);
  ...
  // Tab Navigation
  const tabs = [
    { key: 'upcoming', label: 'Upcoming', count: categorizedAppointments.upcoming?.length || 0 },
    { key: 'today', label: 'Today', count: categorizedAppointments.today?.length || 0 },
    { key: 'past', label: 'Past', count: categorizedAppointments.past?.length || 0 },
  ];
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-blue-600">{categorizedAppointments.upcoming.length}</p>
              </div>
