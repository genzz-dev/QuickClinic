...
const DoctorSchedule = () => {
  ...
  const getWorkingDaysInOrder = () => {
    ...
    return dayOrder.map((day) => schedule.workingDays.find((wd) => wd.day === day)).filter(Boolean);
  };
        ...
        <div className="max-w-6xl mx-auto">
          <div className="bg-white rounded-xl shadow-lg p-8">
            <div className="text-center">
              ...
              <h2 className="text-2xl font-bold text-gray-900 mb-2">No Schedule Found</h2>
      ...
      <div className="max-w-6xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
              ...
              <div className="bg-white rounded-xl shadow-lg p-6">
                ...
                <div className="space-y-3">
                  {schedule.breaks.map((breakItem, index) => (
                    ...
                    >
                      <div className="font-medium text-gray-900 capitalize">{breakItem.day}</div>
                      <div className="text-sm text-gray-600">
                        {formatTime(breakItem.startTime)} - {formatTime(breakItem.endTime)}
                      </div>
                      {breakItem.reason && (
                        <div className="text-sm text-orange-700 mt-1">{breakItem.reason}</div>
                      )}
                    </div>
                  ))}
                </div>
            ...
            {/* Upcoming Vacations */}
            {schedule?.vacations &&
              schedule.vacations.filter((v) => isUpcoming(v.startDate)).length > 0 && (
                <div className="bg-white rounded-xl shadow-lg p-6">
                  <div className="flex items-center mb-4">
                    <Plane className="h-6 w-6 text-purple-600 mr-2" />
                    <h3 className="text-xl font-bold text-gray-900">Upcoming Vacations</h3>
                  </div>
                  <div className="space-y-3">
                    {schedule.vacations
                      ...
                      .map((vacation, index) => (
                        ...
                        >
                          ...
                          {vacation.reason && (
                            <div className="text-sm text-purple-700 mt-1">{vacation.reason}</div>
                          )}
                        </div>
                      ))}
                  </div>
