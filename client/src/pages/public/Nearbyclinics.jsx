...
const NearbyClinicsPage = () => {
  ...
  const availableFilters = useMemo(() => {
    ...
    clinics.forEach((clinic) => {
      ...
      if (clinic.openingHours?.saturday?.open || clinic.openingHours?.sunday?.open) {
  ...
  useEffect(() => {
    ...
    const filtered = clinics.filter((clinic) => {
      ...
      if (filters.doctorAvailable && (!clinic.doctors || clinic.doctors.length === 0)) {
        return false;
      }
      ...
      if (filters.specializations.length > 0) {
        const hasMatchingSpecialization = clinic.doctors?.some((doctorId) => {
          const doctor = clinicDoctors[doctorId];
          return doctor && filters.specializations.includes(doctor.specialization);
        });
      ...
      if (filters.openNow && clinic.openingHours) {
        ...
        if (!todayHours || !isTimeBetween(currentTime, todayHours.open, todayHours.close)) {
          return false;
        }
      }
      ...
      if (filters.openWeekends && clinic.openingHours) {
        const isWeekendOpen =
          clinic.openingHours.saturday?.open && clinic.openingHours.sunday?.open;
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white shadow-sm">
        <div className="px-4 sm:px-6 lg:px-8 py-6">
          ...
          <p className="text-gray-600 mt-2">Found {filteredClinics.length} clinics in your area</p>
        </div>
      </div>
      ...
      <div className="px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="flex-1">
            ...
            {filteredClinics.length === 0 && <NoClinicsFound onRefresh={handleRetry} />}
          </div>
        </div>
      </div>
    </div>
