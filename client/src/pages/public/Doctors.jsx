...
const Doctors = () => {
			...
			if (navigator.geolocation) {
				...
				navigator.geolocation.getCurrentPosition(
					...
					(error) => {
						...
						setLocationError(
							"Unable to get your location. Showing all doctors.",
						);
	...
	const generateFilterOptions = (doctorsData) => {
		...
		const fees = doctorsData
			.map((doctor) => doctor.consultationFee)
			.filter((fee) => fee != null);
		const experiences = doctorsData
			.map((doctor) => doctor.yearsOfExperience)
			.filter((exp) => exp != null);
	...
	useEffect(() => {
		...
		if (filters.teleconsultation) {
			filtered = filtered.filter(
				(doctor) => doctor.availableForTeleconsultation,
			);
		}
		...
		if (filters.minRating > 0) {
			filtered = filtered.filter(
				(doctor) => doctor.averageRating >= filters.minRating,
			);
		}
		...
		if (filters.minExperience > 0) {
			filtered = filtered.filter(
				(doctor) => doctor.yearsOfExperience >= filters.minExperience,
			);
		}
