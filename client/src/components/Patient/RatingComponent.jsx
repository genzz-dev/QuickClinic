...
import { Star, Edit2, Trash2, Check, X, User, Building2 } from 'lucide-react';
import { createRating, updateRating, deleteRating, getDoctorRatings, getClinicRatings } from '../../service/ratingApiService';
...
const RatingComponent = ({ appointmentId, doctorId, clinicId, onRatingUpdate }) => {
    ...
    try {
      ...
      if (doctorId) {
        ...
        const existingDoctorRating = doctorRatings.find(
          (r) => (r.appointmentId?._id === appointmentId || r.appointmentId === appointmentId) && r.ratingType === 'doctor'
        );
      ...
      if (clinicId) {
        ...
        const existingClinicRating = clinicRatings.find(
          (r) => (r.appointmentId?._id === appointmentId || r.appointmentId === appointmentId) && r.ratingType === 'clinic'
        );
  ...
  const handleSubmitRating = async (type) => {
    ...
    try {
      ...
      setError(prev => ({
        ...prev,
        [type]: err.response?.data?.message || `Failed to submit ${type} rating`
      }));
  ...
  const handleDeleteRating = async (type) => {
    ...
    if (!window.confirm(`Are you sure you want to delete this ${type} rating?`)) {
      return;
    }
    ...
    try {
      ...
      setError(prev => ({
        ...prev,
        [type]: err.response?.data?.message || `Failed to delete ${type} rating`
      }));
  ...
  const renderRatingSection = (type) => {
    ...
    const setHoveredStar = isDoctor ? setHoveredDoctorStar : setHoveredClinicStar;
      ...
      return (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <div className="flex items-center gap-3 mb-4">
            <Icon size={24} className={isDoctor ? 'text-blue-600' : 'text-green-600'} />
      ...
      return (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <div className="flex items-center gap-3 mb-4">
            <Icon size={24} className={isDoctor ? 'text-blue-600' : 'text-green-600'} />
            <h3 className="text-lg font-semibold text-gray-900">
              {rating ? 'Edit' : 'Rate'} {type.charAt(0).toUpperCase() + type.slice(1)}
            </h3>
          </div>
          ...
          <form onSubmit={(e) => {
            ...
            <div className="flex gap-3">
              ...
              >
                {loading[type] ? 'Submitting...' : rating ? 'Update Rating' : 'Submit Rating'}
              </button>
              <button
                type="button"
