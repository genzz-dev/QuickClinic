...
import { getDoctorRatings, getClinicRatings } from '../../service/ratingApiService'
...
const StarRating = ({ 
  ...
  if (loading) {
    return (
      <div className={`${inline ? 'inline-flex' : 'flex'} items-center gap-1`}>
        <div className={`${getSizeClasses()} bg-gray-200 rounded animate-pulse`}></div>
  ...
  if (error || ratingData.totalRatingCount === 0) {
    return (
      <div className={`${inline ? 'inline-flex' : 'flex'} items-center gap-1 ${getTextSizeClasses()} text-gray-500`}>
        {renderStars(0)}
        ...
        <div className={`${containerClass} items-center gap-2`}>
          ...
          {showCount && (
            <span className={`${getTextSizeClasses()} text-gray-600`}>
              ({ratingData.totalRatingCount} review{ratingData.totalRatingCount !== 1 ? 's' : ''})
            </span>
          )}
        </div>
        ...
        {ratingData.ratings.length > 0 && (
          <div className="space-y-3">
            ...
            {ratingData.ratings.map((rating, index) => (
              <div key={rating._id || index} className="border-b border-gray-100 pb-3 last:border-b-0">
