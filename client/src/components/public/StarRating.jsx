...
import { getClinicRatings, getDoctorRatings } from '../../service/ratingApiService';
...
const StarRating = ({
  ...
  const renderStars = (rating) => {
    ...
    for (let i = 0; i < 5; i++) {
      if (i < fullStars) {
        stars.push(
          <Star key={i} className={`${getSizeClasses()} fill-yellow-400 text-yellow-400`} />
        );
        ...
        stars.push(<Star key={i} className={`${getSizeClasses()} text-gray-300`} />);
      }
    }
