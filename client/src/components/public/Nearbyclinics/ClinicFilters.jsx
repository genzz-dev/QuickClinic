...
const ClinicFilters = ({ 
  ...
  return (
    <>
      ...
      {/* Filters sidebar */}
      <div className={`${mobileFiltersOpen ? 'block' : 'hidden'} md:block md:w-64 pr-4`}>
        <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 sticky top-4">
          ...
          {showDoctorFilter && (
            <div className="mb-6">
              <h4 className="font-medium text-gray-900 mb-2">Doctor Availability</h4>
              <div className="space-y-2">
                <label className="flex items-center">
                  <input
                    type="checkbox"
                    checked={filters.doctorAvailable}
                    onChange={(e) => onFilterChange('doctorAvailable', e.target.checked)}
                ...
                {showTelemedicineFilter && (
                  <label className="flex items-center">
                    <input
                      ...
                      onChange={(e) => onFilterChange('telemedicine', e.target.checked)}
                      className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                    />
                    <span className="ml-2 text-sm text-gray-700">Offers telemedicine</span>
                  </label>
                )}
              </div>
            </div>
              ...
              {showWeekendFilter && (
                <label className="flex items-center">
                  <input
                    ...
                    onChange={(e) => onFilterChange('openWeekends', e.target.checked)}
                    className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                  />
