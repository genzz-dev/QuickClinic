    ...
    >
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          ...
          <StarRating type="doctor" id={doctor._id} size="small" inline={true} />
          <p className="text-blue-600 font-medium">{doctor.specialization}</p>

          {doctor.bio && <p className="text-gray-600 text-sm mt-1 line-clamp-2">{doctor.bio}</p>}

          <div className="flex flex-wrap items-center gap-4 mt-3 text-sm">
            {doctor.yearsOfExperience && (
              <div className="flex items-center gap-1">
                <Award className="w-4 h-4 text-gray-400" />
                <span className="text-gray-600">{doctor.yearsOfExperience} years exp.</span>
              </div>
            )}
            ...
            {doctor.averageRating > 0 && (
              <div className="flex items-center gap-1">
                ...
                <span className="text-gray-600">({doctor.averageRating.toFixed(1)})</span>
              </div>
            )}
