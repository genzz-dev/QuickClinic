...
const VerificationBanner = ({ clinicData, onVerify, onSetup }) => {
  if (!clinicData) {
    return (
      <div className="bg-gradient-to-r from-gray-50 to-gray-100 border-2 border-dashed border-gray-300 rounded-xl p-8 text-center">
        ...
        <p className="text-gray-600 mb-6">Add your clinic details to unlock verification and management features.</p>
        <button
          onClick={onSetup}
  ...
  const getisVerified = () => {
    ...
    if (attemptsMade >= 3) {
      return {
        ...
        message: 'Max attempts exceeded. Our team will review your clinic details.',
  ...
  return (
    <div className={`${status.bgColor} border-2 ${status.borderColor} rounded-xl p-6`}>
      <div className="flex items-start space-x-4">
        <div className={`flex-shrink-0 p-3 rounded-lg ${status.iconBg}`}>
          <Icon className={`h-6 w-6 ${status.iconColor}`} />
        </div>
        <div className="flex-1 min-w-0">
          <h3 className={`font-semibold text-lg ${status.textColor} mb-1`}>{status.title}</h3>
          <p className={`${status.textColor} opacity-90 mb-2`}>{status.message}</p>
          {status.subMessage && (
            <p className={`text-sm ${status.textColor} opacity-75`}>{status.subMessage}</p>
          )}
          {status.type === 'pending' && (
            <button
              onClick={onVerify}
