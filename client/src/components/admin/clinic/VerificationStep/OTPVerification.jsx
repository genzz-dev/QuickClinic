...
const OTPVerification = ({ onSuccess, onFailed, onManualReview }) => {
	...
	const handleUpdateGoogleLink = async () => {
		...
		try {
			...
			setErrors({
				googleLink:
					error.response?.data?.message || "Failed to update Google Maps link",
			});
		} finally {
			...
			{showGoogleLinkForm && (
				<div className="mb-8 p-6 bg-yellow-50 rounded-lg border border-yellow-200">
					...
					<p className="text-yellow-700 mb-4">
						To verify your clinic via OTP, we need your Google Maps business
						listing link.
					</p>
			...
			<div className="space-y-6">
				{!otpSent ? (
					<div>
						<p className="text-gray-700 mb-6">
							Click below to send an OTP to your clinic's registered phone
							number
						</p>
						<button
						...
						{!hasGoogleLink && (
							<p className="text-orange-600 text-sm mt-2 text-center">
								Please add your Google Maps link first to enable OTP
								verification
							</p>
						)}
					</div>
					...
					<div>
						<p className="text-green-600 mb-4 flex items-center">
							<Shield className="h-4 w-4 mr-2" />✓ OTP sent successfully! Check
				...
				<div className="bg-yellow-50 p-4 rounded-md border border-yellow-200">
					<p className="text-yellow-800 text-sm">
						...
						Unverified clinics won't display the verification badge until manual
