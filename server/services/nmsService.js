...
export const checkDoctorName = async (inputName, regNumber, regYear, stateCouncil) => {
    let browser = null;
    ...
    try {
        ...
        await page.waitForSelector('a[href="#advanceSearch"]', { timeout: TIMEOUT });
        ...
        if (stateCouncil) {
            ...
            const councilOptions = await page.evaluate(() => {
                ...
                return Array.from(select.options).map(option => ({ value: option.value, text: option.text }));
            });
            
            const validOption = councilOptions.find(option => option.value === councilId.toString());
            if (!validOption && councilId !== '') {
                // Try to find by partial text match
                const textMatch = councilOptions.find(option => 
                    option.text.toLowerCase().includes('gujarat') || 
                    option.text.toLowerCase().includes(stateCouncil.toLowerCase().split(' ')[0])
                );
                if (textMatch) {
        ...
        try {
            ...
            await page.waitForFunction(() => {
                const processingDiv = document.querySelector('#doct_info5_processing');
                const isProcessing = processingDiv && processingDiv.style.display !== 'none';
                ...
                const hasNoDataMessage = noDataMsg && noDataMsg.style.display !== 'none';
            ...
            return { match: false, error: "Search timeout - no results or error message appeared" };
        }
