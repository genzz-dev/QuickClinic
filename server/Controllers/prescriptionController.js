		...
		medications.forEach((med, index) => {
			if (!med.name || !med.dosage || !med.frequency || !med.duration) {
				medicationErrors.push(
					`Medication ${index + 1} is missing required fields`,
				);
			}
		});
...
export const updatePrescription = async (req, res) => {
	try {
		...
		if (
			role !== "doctor" ||
			prescription.doctorId.toString() !== profileId.toString()
		) {
			return res.status(403).json({ message: "Unauthorized" });
		}
	...
	try {
		...
		if (
			...
			return res
				...
				.json({
					message:
						"Unauthorized - You can only view prescriptions for your own appointments",
				});
		}
