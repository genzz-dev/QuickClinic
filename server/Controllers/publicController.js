...
export const searchDoctors = async (req, res) => {
  try {
    ...
    switch(sort) {
      ...
      case 'fee_desc': sortOption = { consultationFee: -1 }; break;
...
export const getDoctorAvailability = async (req, res) => {
  try {
    ...
    if (!doctor) return res.status(404).json({ success: false, error: 'Doctor not found' });
    if (!doctor.schedule) return res.json({ success: true, data: { available: false, reason: 'No schedule' } });
    ...
    const isOnVacation = doctor.schedule.vacations.some(v => 
      requestedDate >= new Date(v.startDate) && requestedDate <= new Date(v.endDate)
    );
    if (isOnVacation) return res.json({ success: true, data: { available: false, reason: 'On vacation' } });

    // Check working day
    const workingDay = doctor.schedule.workingDays.find(d => 
      d.day.toLowerCase() === dayName && d.isWorking
    );
    if (!workingDay) return res.json({ success: true, data: { available: false, reason: 'Not working today' } });

    // Generate available slots
    const dayBreaks = doctor.schedule.breaks.filter(b => b.day.toLowerCase() === dayName);
    const allSlots = generateTimeSlots(workingDay.startTime, workingDay.endTime, doctor.schedule.appointmentDuration);
  ...
  try {
    ...
    const [doctors, total] = await Promise.all([
      Doctor.find({ clinicId })
        .skip(skip)
        .limit(parseInt(limit))
        .select('firstName lastName specialization profilePicture consultationFee averageRating'),
...
export const checkDoctorAvailability = async (req, res) => {
  try {
    ...
    if (requestedTime < workStart || requestedTime >= workEnd) {
      return res.json({ success: true, data: { available: false, reason: 'Outside working hours' } });
    }
    ...
    if (isInBreak) return res.json({ success: true, data: { available: false, reason: 'During break' } });
...
export const getSearchSuggestions = async (req, res) => {
  try {
    ...
    const [doctors, clinics] = await Promise.all([
      ...
      }).limit(5).select('firstName lastName specialization'),
      ...
      }).limit(5).select('name address.city address.state')
    ]);
