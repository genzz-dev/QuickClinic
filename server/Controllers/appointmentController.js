...
export const bookAppointment = async (req, res) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(doctorId) || !mongoose.Types.ObjectId.isValid(profileId)) {
    ...
    if (!doctor.clinicId) {
      return res.status(400).json({
        message: 'Doctor is not associated with any clinic. Cannot book appointment.',
      });
    }
...
export const getPatientInfoForAppointment = async (req, res) => {
  try {
    ...
    // Get patient info
    const patient = await Patient.findById(appointment.patientId).populate('healthRecords').lean();
    ...
    const age = Math.floor(
      (Date.now() - new Date(patient.dateOfBirth)) / (365.25 * 24 * 60 * 60 * 1000)
    );
