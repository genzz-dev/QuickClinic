...
export const createAdminProfile = async (req, res) => {
	try {
		...
		if (existingAdmin) {
			return res
				.status(409)
				.json({ message: "Admin profile already exists for this user" });
		}
	...
	try {
		...
		if (clinicData.googleMapsLink) {
			try {
				...
				const placeDetails = await withRateLimit(() =>
					fetchPlaceDetails(placeId),
				);

				clinicData.address = {
					formattedAddress:
						cleanFormattedAddress(placeDetails.adr_address) || "",
				...
				const googlePhotos = await withRateLimit(() =>
		...
		if (req.files?.logo) {
			try {
				...
				return res
					.status(500)
					.json({ message: "Failed to upload clinic logo" });
			}
		}
...
export const updateClinic = async (req, res) => {
	try {
		...
		for (const key in unflattenedUpdates) {
			...
			if (key === "facilities") {
				const facilities = Array.isArray(unflattenedUpdates.facilities)
					? unflattenedUpdates.facilities
					: [unflattenedUpdates.facilities];
		...
		// Final validation.
		if (
			updateObj.$set.name !== undefined &&
			updateObj.$set.name?.trim() === ""
		) {
			return res
				.status(400)
				.json({
					message: "Validation error",
					errors: ["Clinic name cannot be empty"],
				});
		}

		const updatedClinic = await Clinic.findByIdAndUpdate(clinicId, updateObj, {
			new: true,
			runValidators: true,
		}).select("-__v");
		...
		if (error.codeName === "ConflictingUpdateOperators") {
			return res
				.status(400)
				.json({
					message: `A conflict occurred while trying to update fields. Details: ${error.errmsg}`,
				});
		}
		res.status(500).json({
...
export const addDoctorToClinic = async (req, res) => {
	try {
		...
		if (doctorInClinic) {
			return res
				.status(409)
				.json({ message: "Doctor is already associated with this clinic" });
		}
...
export const setDoctorSchedule = async (req, res) => {
	try {
		...
		if (workingDays) {
			...
			workingDays.forEach((day) => {
				...
				if (day.isWorking && (!day.startTime || !day.endTime)) {
					dayErrors.push(
						`Working day ${day.day} must have start and end times`,
					);
				}
			});
		...
		if (
			appointmentDuration &&
			(isNaN(appointmentDuration) || appointmentDuration <= 0)
		) {
			return res
				.status(400)
				.json({ message: "Appointment duration must be a positive number" });
		}
...
export const getDoctorSchedule = async (req, res) => {
	try {
		...
		const schedule = await Schedule.findOne({ doctorId }).populate("doctorId");
	...
	try {
		...
		if (clinic.verificationAttempts >= 3) {
			return res.status(429).json({
				message:
					"Maximum verification attempts reached. Verification is no longer possible.",
			});
		}
...
export const verifyClinicOTP = async (req, res) => {
	try {
		...
		if (!clinic.googleMapsPhone) {
			return res
				.status(400)
				.json({ message: "No phone number found for verification" });
		}
