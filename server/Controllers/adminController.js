...
export const addClinic = async (req, res) => {
  try {
    ...
    await Admin.findOneAndUpdate({ userId }, { clinicId: clinic._id }, { new: true });
...
export const addDoctorToClinic = async (req, res) => {
  try {
    ...
    await Doctor.findByIdAndUpdate(doctorId, { clinicId }, { runValidators: true });
...
export const setDoctorSchedule = async (req, res) => {
  try {
    ...
    await Doctor.findByIdAndUpdate(doctorId, { schedule: schedule._id }, { new: true });
...
export const removeDoctorFromClinic = async (req, res) => {
  try {
    ...
    await Doctor.findByIdAndUpdate(doctorId, { $unset: { clinicId: 1 } }, { new: true });
